...
@router.post("/add_new")
def add_user(new_user:User_pydantic, db: Session=Depends(db_add), _=Depends(auth.get_admin)):
...
def add_group(new_group: Group, db: Session=Depends(db_add), _=Depends(auth.get_elder)):
    to_dict=new_group.model_dump()
    group_info= user.create_new_group(to_dict, db)
    return group_info

@router.get("/view_groups")
def view_groups(db: Session=Depends(db_add),  _=Depends(auth.get_elder)):
    groups=user.view_groups(db)
    return groups

@router.delete("/delete_group/{id}")
def delete_group(id: int, db: Session=Depends(db_add), _=Depends(auth.get_admin)):
    return user.delete_group(id, db)

@router.get("/all_group_members/{id}")
def get_all_members(id: int, db:Session=Depends(db_add), _=Depends(auth.get_elder)):
